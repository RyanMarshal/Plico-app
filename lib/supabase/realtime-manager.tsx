...
export function RealtimeManagerProvider({ children }: { children: React.ReactNode }) {
  ...
  const retryConnection = useCallback((config: ChannelConfig, attempt: number = 0) => {
    ...
    if (isRetryingRef.current.has(config.channelName)) {
      console.log(`[RealtimeManager] Already retrying ${config.channelName}, skipping duplicate retry`)
      return
    }
  ...
  const createSubscription = useCallback((config: ChannelConfig, retryAttempt: number = 0) => {
    ...
    try {
      ...
      const subscribeTimeout = setTimeout(() => {
        console.warn('[RealtimeManager] Subscribe timeout for', config.channelName)
        channel.unsubscribe()
  ...
  const subscribe = useCallback((config: ChannelConfig) => {
    ...
    if (existing) {
      console.log('[RealtimeManager] Reusing existing channel:', config.channelName)
      // Return a no-op cleanup since we're reusing the channel
      // In a production app, you'd want reference counting here
      return () => {
        console.log('[RealtimeManager] Cleanup called for shared channel:', config.channelName)
  ...
  const forceReconnect = useCallback(() => {
    ...
    channelsRef.current.forEach(({ channel, config }) => {
      try {
        ...
        console.warn('[RealtimeManager] Error removing channel during reconnect:', e)
      }
      createSubscription(config)
    })
  }, [createSubscription])
    ...
    return () => {
      ...
      channelsRef.current.forEach(({ channel }) => {
        try {
          ...
          console.warn('[RealtimeManager] Error removing channel during cleanup:', e)
        }
      })
